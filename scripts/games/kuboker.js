...
function getSuggestedDc(state) {
  const sourceEntries = Object.entries(state?.players ?? {}).filter(([, entry]) => entry?.isParticipating);
  const levels = sourceEntries.map(([actorId, entry]) => {
    const actor = game.actors?.get(actorId);
    const actorLevel = Math.trunc(Number(actor?.level ?? actor?.system?.details?.level?.value ?? Number.NaN));
    const storedLevel = Math.trunc(Number(entry?.level ?? 0));
...
function buildEmptyState(definition, state, visibleEntries = null) {
  ...
  if (state.phase === "join" && !participatingCount) {
    return gt(definition, "Empty.JoinInstructions", isRussianLocale() ? "Игроки отмечают «Я в игре», затем ГМ нажимает «Начать»." : "Players mark \"I'm in the game\", then the GM clicks Start.");
  }
  if (state.phase !== "join" && !participatingCount) {
...
function buildSummaryUi(definition, state) {
  return {
    ...
    debugLabel: gt(definition, "DebugMode", isRussianLocale() ? "Режим ГМ" : "GM Mode"),
    helpTitle: gt(definition, "Rules.HelpTitle", "Kuboker Rules"),
    helpHtml: gt(definition, "Rules.HelpHtml", "<p>No rules available.</p>"),
    buttons: {
      start: gt(definition, "Buttons.Start", isRussianLocale() ? "Начать" : "Start"),
      round: gt(definition, "Buttons.ResetRound", isRussianLocale() ? "Раунд" : "Round"),
      reveal: gt(definition, "Buttons.Reveal", "Reveal"),
      clear: gt(definition, "Buttons.Clear", isRussianLocale() ? "Очистить" : "Clear"),
      resetGame: gt(definition, "Buttons.ResetGame", isRussianLocale() ? "Сброс" : "Reset"),
...
class KubokerApplication extends Application {
  ...
  getData() {
    ...
    if (game.user.isGM) {
      const roundButton = {
        action: "deal",
        label: state.phase === "join" ? ui.buttons.start : ui.buttons.round,
        title: state.phase === "join" ? ui.buttons.start : ui.buttons.round,
        classes: "is-deal",
      ...
      if (state.phase === "betting") {
        ...
        roundButton.label = ui.buttons.round;
      ...
      footerButtons.push({ action: "clear", label: ui.buttons.clear, title: ui.buttons.clear, classes: "is-clear" });
      footerButtons.push({ action: "reset-game", label: ui.buttons.resetGame, title: ui.buttons.resetGame, classes: "is-reset" });
    }
