...
const MANUAL_SUPPORT_DC_ROUND_OFFSETS = Object.freeze([-2, 2, 6]);
...
const TEXT = Object.freeze({
  ...
  FooterStart: { ru: "Начать", en: "Start" },
...
function getSuggestedTechniqueBaseLevel(state) {
  ...
  return Math.min(...entries.map(([actorId, playerData]) => {
    const actor = game.actors?.get(actorId);
    const actorLevel = getActorLevel(actor);
    const storedLevel = Math.max(0, Math.trunc(Number(playerData?.level ?? 0) || 0));
    const level = actor ? actorLevel : storedLevel;
  ...
  getData() {
    ...
    if (game.user?.isGM) {
      const primary = {
        ...
        label: state.phase === "join" ? tx("FooterStart") : tx("FooterRound"),
        classes: "is-main",
