import { TaleSaleJournalSheet } from "./base.js";

const LAYOUT_ROOT = "/modules/ts-pf2e-utility/layout/СС";

export class CCJournalSheet extends TaleSaleJournalSheet {
  static get journalSheetConfig() {
    return {
      className: "CC",
      variablePrefix: "cc",
      titleTextClass: "cc-window-title-text",
      frameImage: `${LAYOUT_ROOT}/border.webp`,
      sidebarImage: `${LAYOUT_ROOT}/back.png`,
      textColor: "#1f1a17",
      h1Color: "#b51827",
      h2Color: "#b51827",
      h3Color: "#b51827",
      h4Color: "#2b231d",
      traitColor: "rgb(124, 52, 46)",
      gmVisibilityBackground: "#dcdce3",
      blockquoteTextColor: "rgb(124, 52, 46)",
      blockquoteBackgroundColor: "#f8f8eb",
      blockquoteRuleColor: "rgb(124, 52, 46)",
      sceneEyeTextColor: "#5f040d",
      sidebarTextColor: "#1f1a17",
      sidebarActiveTextColor: "#5f040d",
      sidebarCategoryColor: "#b51827",
      themeVariables: {
        "header-image": `url('${LAYOUT_ROOT}/back.png')`,
        "title-image": `url('${LAYOUT_ROOT}/back.png')`,
        "insite-image": `url('${LAYOUT_ROOT}/back.png')`,
      },
      cssVariables: {
        "page-content-pad-left": "10px",
        "page-content-pad-right": "6px",
        "scrollbar-width": "8px",
        "header-right-adjust": "1px",
        "frame-top": "-36px",
        "frame-right": "-15px",
        "frame-bottom": "-15px",
        "frame-left": "-30px",
        "frame-outset-top": "36px",
        "frame-outset-right": "15px",
        "frame-outset-bottom": "15px",
        "frame-outset-left": "15px",
        "resize-size": "24px",
      },
    };
  }
}
